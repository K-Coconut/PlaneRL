...
class Rank_Replay_Buffer:
    ...
    def __init__(self, capacity=int(1e6), batch_size=64):
        ...
        self.segments = [-1] + [None for _ in
                                range(batch_size)]  # the ith index will be in [segments[i-1]+1, segments[i]]
    ...
    def sample(self, batch_size=None):  # notive that batch_size is not used. It's just to unify the calling form
        ...
        probs = torch.from_numpy(
            np.vstack([(1 / (1 + ind)) ** self.alpha / self.total_weights for ind in index_set])).float()
...
class Proportion_Replay_Buffer:
    ...
    def sample(self, batch_size):
        ...
        # print(index_set)
        probs = torch.from_numpy(
            np.vstack([self.weights.vals[ind + self.capacity - 1] / self.weights.vals[0] for ind in index_set])).float()
